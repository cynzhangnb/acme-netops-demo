...
const CHANGES = [
  {
    ...
    device: 'CR-BOS-02',
    type: 'BGP Policy',
    description: 'BGP policy update — route preference modified for voice VLAN traffic',
    timestamp: '2026-03-29 14:32 UTC',
...
route-policy VOICE-IN-V2
 ...
 pass
    ...
    changedLines: { before: [3, 5, 13, 14], after: [3, 5, 13, 14] },
  },
  {
    id: 2,
    device: 'CR-BOS-02',
    type: 'VLAN Route',
    description: 'Voice VLAN static route — next-hop updated for 10.8.3.0/24',
    timestamp: '2026-03-29 14:35 UTC',
...
function SkeletonRow() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', padding: '14px 20px', borderBottom: '1px solid #f5f5f5', gap: 16 }}>
      <div style={{ flex: 2, display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={{ height: 10, width: '55%', borderRadius: 4, background: '#ececec', animation: 'skeleton-pulse 1.4s ease-in-out infinite' }} />
        <div style={{ height: 8, width: '35%', borderRadius: 4, background: '#f3f3f3', animation: 'skeleton-pulse 1.4s ease-in-out infinite 0.1s' }} />
      </div>
      <div style={{ flex: 3 }}>
        <div style={{ height: 9, width: '85%', borderRadius: 4, background: '#ececec', animation: 'skeleton-pulse 1.4s ease-in-out infinite 0.05s' }} />
      </div>
      <div style={{ flex: 1.5 }}>
    ...
    <div ref={containerRef} style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ padding: '14px 20px 10px', borderBottom: '1px solid #e8e8e8', flexShrink: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#111' }}>2 Change Events</div>
        </div>

        {/* Column headers */}
        <div style={{ display: 'flex', padding: '6px 20px', borderBottom: '1px solid #f0f0f0', background: '#fafafa', flexShrink: 0 }}>
          <span style={{ flex: 2, fontSize: 11, fontWeight: 500, color: '#888' }}>DEVICE / TYPE</span>
          <span style={{ flex: 3, fontSize: 11, fontWeight: 500, color: '#888' }}>DESCRIPTION</span>
          <span style={{ flex: 1.5, fontSize: 11, fontWeight: 500, color: '#888' }}>TIMESTAMP</span>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {isLoading ? (
            ...
            return (
              ...
              >
                <div style={{ flex: 2 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#111' }}>{change.device}</div>
                  <div style={{ fontSize: 11, color: '#888', marginTop: 1 }}>{change.type}</div>
                </div>
                <div style={{ flex: 3, fontSize: 12, color: '#333' }}>{change.description}</div>
                <div style={{ flex: 1.5, fontSize: 11, color: '#666' }}>{change.timestamp}</div>
              </div>
            )
          })}
        </div>
